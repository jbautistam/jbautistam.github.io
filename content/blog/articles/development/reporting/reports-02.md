...
Además, pensando en un informe no solicitado aún ¿cuáles serían las ventas totales? No son más que la
...
posiblemente sigue existiendo en cuanto a tamaño de la tabla.
...
```sql
SELECT Stores.Name, Calendar.Date, SUM(Sales.Quantity) AS Quantity, SUM(Sales.Price) AS Price
	FROM Fact.Sales INNER JOIN Dim.Calendar
		ON Sales.CalendarId = Calendar.Id
...
```sql
SELECT Stores.Name, Products.Name, Calendar.Date, SUM(Sales.Quantity) AS Quantity, SUM(Sales.Price) AS Price
	FROM Fact.Sales INNER JOIN Dim.Calendar
		ON Sales.CalendarId = Calendar.Id
	INNER JOIN Dim.Stores
		ON Sales.StoreId = Stores.Id
...
Si las miramos en su conjunto, no parecen tampoco excesivamente complejas. Son sólo cadenas a las que vamos añadiendo `JOIN` y filtros.
Repito, si algo tiene bueno el SQL es que sus comandos son exclusivamente cadenas de texto.

Sería posible partir de este tipo de estructuras y ponernos a trabajar en nuestra solución, pero vamos a intentar dar un paso más, vamos
...
WITH CalendarCte AS
	...
	SELECT StoresCte.StoreName, ProductsCte.ProductName, CalendarCte.Date, SUM(Sales.Quantity) AS Quantity,
...
los hemos sustituido por `StoreName` y `City` (en cualquier caso, estamos obligados por el lenguaje, una CTE no nos permite tener dos campos con el mismo
nombre). 
...
WITH CalendarCte AS
	...
	SELECT ProductsCte.ProductName, CalendarCte.Date, SUM(Sales.Quantity) AS Quantity,
		   SUM(Sales.Price) AS Price
		FROM Fact.Sales INNER JOIN CalendarCte
			ON Sales.CalendarId = CalendarCte.Id
		INNER JOIN ProductsCte
...
calendario 445, directamente no existe la función.
